...
size_t write_callback(char *ptr, size_t size, size_t nmemb, void *userdata)
{
    ...
    if (new_data == NULL)
    {
        perror(BHRED "Erro de alocação de memoria:" RESET);
        return 0;
    }
...
void extraiHTML(Serie serie[], int i)
{
    ...
    if (curl)
    {
        ...
        if (res != CURLE_OK)
        {
            printf(BHRED "Erro na solicitação: %s\n" RESET, curl_easy_strerror(res)); // caso n tenha dado certo ele avisara no terminal e encerrara o programa
            return 0;
        }
    }
    ...
    if (arq == NULL)
    {
        perror(BHRED "Erro na abertura do arquivo" RESET);
    ...
    if (resultado == EOF)
    {
        perror(BHRED "Erro na gravação do arquivo" RESET);
    }
